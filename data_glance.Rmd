...
mean(abs(resid(mod_central_distrcits_Sample)))
median(abs(resid(mod_central_distrcits_Sample)))

```

________________
Individual Residual and MSE for Suburb Model, CI,PI
...
#Mean and median residual
#abs(resid(mod_chaoyang_Sample))
mean(abs(resid(mod_suburb_Sample)))
median(abs(resid(mod_suburb_Sample)))

```

________________
Assessing Conditions for all model

```{r}
#Assessing Conditions
plot(mod_chaoyang_full, which=c(1,2))
plot(mod_central_distrcits, which=c(1,2))
plot(mod_suburb, which=c(1,2))
