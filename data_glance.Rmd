...
```{r}
mdpattern(listings_numerical)
```

------------------------
```{r}
listings_numerical_clean <- listings_numerical%>%
  select(-c(square_feet, weekly_price,monthly_price))
```

For the sake of testing and seeing an R value, I'm dropping everything with N/A

```{r}
listings_numerical_test <- listings_numerical%>%
  select(-c(square_feet, weekly_price,monthly_price,zipcode,security_deposit,
...
mfull_test <- lm(price_per_p ~ ., data=listings_numerical_test) # takes pretty much forever to run 
summary(mfull_test)
```
...
mamenities_test <- lm(price_per_p ~ ., data = listings_numerical_test)
summary(mamenities_test) # I don't even want to comment on the results
```
